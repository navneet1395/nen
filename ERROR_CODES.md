...
Every failure inside the Nen layer carries a stable `ISO-xxxx` code. **This is
the single source of truth** — the TypeScript catalogs in
`packages/nen-server/src/errors.ts` and `packages/nen-client/src/errors.ts`
mirror this table.

## Why codes exist

When an Nen request fails, the cause is almost always in *our* layer — key
...
| :--- | :--- | :--- | :--- |
| ISO-1001 | 400 | `HANDSHAKE_MISSING_PUBLIC_KEY` | Handshake body had neither `pk` nor `publicKey`. Client SDK out of date, or the request wasn't made by an Nen client. |
...
| ISO-1003 | 503 | `HANDSHAKE_NETWORK` | Couldn't reach `/api/nen/handshake`. Wrong `serverUrl`, server down, or CORS. |
...
| Code | HTTP | Name | What it means / what to check |
| :--- | :--- | :--- | :--- |
| ISO-3001 | 401 | `AUTH_SIGNATURE_MISSING` | No `X-Nen-Signature` on a session that requires HMAC. **HMAC is mandatory** — this is the auth-downgrade guard. |
...
| Code | HTTP | Name | What it means / what to check |
| :--- | :--- | :--- | :--- |
| ISO-6001 | 400 | `WIRE_INVALID_PAYLOAD_FORMAT` | Body was missing the `(ct, n)` base64 pair. Not an Nen payload, or a corrupted/truncated body. |
...
| Code | HTTP | Name | What it means / what to check |
| :--- | :--- | :--- | :--- |
| ISO-7001 | 502 | `STREAM_MISSING_NONCE_HEADER` | Stream response had no `X-Nen-Stream-Nonce`. The route didn't use `withNenStream()`, or a proxy stripped it. |
...
| Code | HTTP | Name | What it means / what to check |
| :--- | :--- | :--- | :--- |
| ISO-9000 | 500 | `INTERNAL` | Unclassified failure wrapped by `NenError.from()`. The original error is in the logged `detail`. |

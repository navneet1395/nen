...
From zero to an encrypted API call in about ten lines. TLS stays exactly as it is — Isogeny adds the layer that survives TLS termination.
...
This detects your Next.js App Router project, installs `@isogeny/client` and `@isogeny/server`, patches `next.config.ts` for WebAssembly, and generates the session routes.
...
`src/app/api/isogeny/[action]/route.ts`:

```typescript
import { handleHandshake, handleRotate, handleTerminate, handleStatus } from '@isogeny/server';
...
- [Threat model](/docs/threat-model) — what Isogeny does and does not protect.

...
- Implementations come from the audited **RustCrypto** crates — Isogeny does not hand-roll primitives.
- No custom transport crypto: Isogeny runs above TLS.
...
- **7** client tests — coded error paths, `IsogenyError` shape, and the guarantee that the wire body never leaks the internal diagnostic hint.

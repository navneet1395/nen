...
Integrating Nen is four steps: mount the session routes, encrypt requests from the client, wrap the endpoints you want decrypted, and handle errors. All snippets below match the shipped `@nen/client` and `@nen/server` APIs.
...
```typescript
import { NenError } from '@nen/client'; // or '@nen/server'

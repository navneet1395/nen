...
import { Lock, Unlock, Zap, ShieldCheck, Server, MonitorSmartphone, TerminalSquare, Play, RotateCcw } from "lucide-react";

export function InteractiveDemo() {
  const [step, setStep] = useState(0);
  const [logs, setLogs] = useState<string[]>([]);
  const [isPlaying, setIsPlaying] = useState(false);
  const [customPayload, setCustomPayload] = useState('{"balance": 1500.00, "user_id": "usr_99X"}');
  ...
  const steps = [
    ...
    { label: "Derive Keys", desc: "Derive AEAD Session Keys on Client & Server" },
  ...
  const resetDemo = () => {
    setStep(0);
    setIsPlaying(false);
    setLogs([`[SYSTEM] Configured: ML-KEM-${kemLevel} + ${symmetricAlgo}. Ready to initiate handshake.`]);
  };
  ...
  const addLog = (msg: string) => {
    setLogs((prev) => {
      const newLogs = [...prev, `[${new Date().toISOString().substring(11, 23)}] ${msg}`];
  ...
  const runHandshake = async () => {
    ...
    setStep(2);
    addLog(`[NETWORK] POST /api/isogeny/handshake - Transmitting Client Public Key.`);
    addLog(`[NETWORK] Payload: { pk_len: ${kemLevel === "512" ? 800 : kemLevel === "768" ? 1184 : 1568}B, algo: "ML-KEM-${kemLevel}" }`);
    ...
    await new Promise(r => setTimeout(r, 600));
    addLog(`[CLIENT] Decapsulation successful. Both sides derived identical Shared Secret.`);
    addLog(`[SYSTEM] Derived ${symmetricAlgo} Symmetrical Session Keys.`);

    // Step 4 -> 5: Secure Send (Payload Transmission)
    await new Promise(r => setTimeout(r, 1200));
    setStep(5);
    addLog(`[CLIENT] Encrypting payload with derived ${symmetricAlgo} session key.`);
  ...
  return (
    <Card ref={demoRef} className="w-full max-w-4xl mx-auto shadow-2xl border-border bg-background/50 backdrop-blur-md overflow-hidden flex flex-col relative">
      <CardHeader className="border-b bg-muted/10 pb-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription className="mt-1.5">Interactive Post-Quantum Key Exchange & Payload Encryption</CardDescription>
          </div>
      ...
      <div className="p-5 border-b bg-muted/5 grid grid-cols-1 md:grid-cols-4 gap-4 items-end text-sm">
        <div className="flex flex-col gap-1.5 md:col-span-2">
          <label className="font-semibold text-xs text-muted-foreground uppercase tracking-wider">Plaintext Payload to Encrypt</label>
          <input 
            type="text" 
            value={customPayload}
            disabled={isPlaying}
        ...
        <div className="flex flex-col gap-1.5">
          <label className="font-semibold text-xs text-muted-foreground uppercase tracking-wider">ML-KEM Level</label>
          <select 
            value={kemLevel}
            disabled={isPlaying}
        ...
        <div className="flex gap-2">
          <Button 
            ...
            disabled={isPlaying}
            size="sm"
            className="flex-1 text-xs bg-primary hover:bg-primary/90 text-primary-foreground font-semibold flex items-center justify-center gap-1.5 shadow-md"
          >
      ...
      <CardContent className="p-0 flex flex-col">
        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-4 p-6 items-stretch bg-gradient-to-b from-transparent to-muted/5 relative">
          ...
          {/* Client Side Panel */}
          <div className={"flex flex-col border rounded-xl bg-background/80 shadow-sm transition-all duration-500 overflow-hidden " + (step >= 1 ? 'ring-2 ring-primary/50 border-primary/20' : 'border-border/50')}>
                ...
                <div className="space-y-2.5 text-xs font-mono">
                  <div>
                    <div className="flex justify-between items-center text-muted-foreground mb-1">
                      ...
                      <span className="text-[10px] bg-primary/10 text-primary px-1.5 py-0.5 rounded">{kemLevel === "512" ? "800" : kemLevel === "768" ? "1184" : "1568"} bytes</span>
                    </div>
          ...
          {/* Server Side Panel */}
          <div className={"flex flex-col border rounded-xl bg-background/80 shadow-sm transition-all duration-500 overflow-hidden " + (step >= 3 ? 'ring-2 ring-primary/50 border-primary/20' : 'border-border/50')}>
            <div className="bg-muted/30 p-3 border-b flex items-center gap-2 font-medium text-sm">
        ...
        {/* Faux Terminal Logs */}
        <div className="bg-[#0D1117] text-[#E6EDF3] p-4 font-mono text-xs border-t min-h-[140px] flex flex-col justify-end">
           <div className="flex items-center gap-2 mb-2 text-[#7D8590] pb-2 border-b border-[#30363D]">
             <TerminalSquare className="w-4 h-4" />
             <span>isogeny-engine.log</span>
           </div>
           <div className="space-y-1 overflow-hidden">
             {logs.map((log, i) => (
               <div key={i} className={log.includes("[SYSTEM]") ? "text-primary/95" : log.includes("[SERVER]") ? "text-green-400" : log.includes("[CLIENT]") ? "text-blue-400" : log.includes("[NETWORK]") ? "text-orange-400" : "text-[#E6EDF3]"}>
                 {log}
               </div>
             ))}
           </div>

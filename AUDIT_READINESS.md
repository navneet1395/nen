...
- ✅ Implementations come from the audited **RustCrypto** crates (`ml-kem`,
  `chacha20poly1305`, `hmac`, `sha2`, `ml-dsa`) — Isogeny does not hand-roll
  primitives.
- ✅ No custom transport crypto: Isogeny runs above TLS (see threat model §1).

## 2. Test coverage

- ✅ 16 Rust unit tests in `core-crypto` (KEM round-trip, AEAD tamper detection,
  HMAC sign/verify, base64, signatures).
- ✅ Server suite (`@isogeny/server`): 17 tests — handshake, session lifecycle,
...
- ✅ Client suite (`@isogeny/client`): 7 tests — coded error paths (`ISO-2001`,
  `ISO-1003`, `ISO-1004`), `IsogenyError` shape, and the safe-body / no-hint-leak
...
2. Independent cryptographic review of `ISOGENY-PROTOCOL-V1` (handshake, HMAC hot

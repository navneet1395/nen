...
describe('UpstashSessionStore (REST contract)', () => {
  ...
  const hmacKey = new Uint8Array(32).fill(4);
  ...
  test('round-trips a session and tracks nonces over REST', async () => {
    ...
    await store.set(sid, sharedSecret, hmacKey);
    expect(await store.exists(sid)).toBe(true);

    const got = await store.get(sid);
    expect(got).not.toBeNull();
    expect(Array.from(got!.sharedSecret)).toEqual(Array.from(sharedSecret));
    expect(Array.from(got!.hmacKey)).toEqual(Array.from(hmacKey));

const nenCrypto = require('../pkg/node/core_crypto.js');
...
async function runPacketTrace() {
  ...
  console.log(`[NETWORK] 2. Transmitting POST /api/nen/handshake`);
  ...
  const hsRes = await fetch(`${TARGET_URL}/api/nen/handshake`, {
    method: 'POST',
  ...
  console.log(`\n[NETWORK] 3. Received Response from /api/nen/handshake (${hsTime} ms)`);
  ...
  const sharedSecret = nenCrypto.nen_decapsulate(new Uint8Array(hsData.ciphertext), keypair.secret_key);
  ...
  const ciphertext = nenCrypto.nen_encrypt(sharedSecret, nonce, payloadBytes);
  ...
  const decryptedBytes = nenCrypto.nen_decrypt(
    sharedSecret, 
